...
export default function CallForm({ initial, onSuccess, onCancel }: CallFormProps) {
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    fd.append('location', values.is_online ? 'Online' : (values.location ?? ''));
    fd.append('is_online', values.is_online ? '1' : '0');
    if (values.max_participants) fd.append('max_participants', values.max_participants);
  ...
  const prereqOptions = PREREQUISITES.map((p) => ({ value: p.value, label: p[lang] }));
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4" noValidate>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <Controller
            ...
            render={({ field }) => (
              <Select
                ...
                options={CALL_TYPES.map((c) => ({ value: c, label: CALL_TYPE_LABELS[c][lang] }))}
              />
            )}
          />
        </Field>
        ...
        <Field label={`${t('common.deadline')} *`} error={errors.application_deadline?.message}>
          <Controller
            control={control}
            name="application_deadline"
            render={({ field }) => <DateField value={field.value} onChange={field.onChange} withTime />}
          />
        </Field>
        <Field label="Price (€)">
          <input type="number" min="0" step="0.01" className="input" {...register('price')} />
        </Field>
        <Field label="Start date">
          <Controller
            ...
            name="start_date"
            render={({ field }) => (
              <DateField value={field.value ?? ''} onChange={field.onChange} withTime placeholder="—" />
            )}
          />
        </Field>
        ...
        <Field label="Max participants">
          <input type="number" min="1" className="input" {...register('max_participants')} />
        </Field>
        <div className="flex items-end">
          <label className="flex items-center gap-2 text-sm">
            <input type="checkbox" className="h-4 w-4 accent-brand-600" {...register('is_online')} />
            {t('common.online')}
          </label>
        </div>
      </div>
      ...
      <Field label={`${t('browse.category')} *`} error={errors.categories?.message}>
        <Controller
          control={control}
          name="categories"
          render={({ field }) => (
