    ...
    >
      <div className="relative h-40 overflow-hidden">
        {call.image ? (
          ...
          <div className={`flex h-full w-full items-center justify-center bg-gradient-to-br ${gradient}`}>
            <span className="text-3xl font-extrabold text-white/90">{typeLabel}</span>
          </div>
        )}
        <span className="absolute left-3 top-3 chip bg-white/90 text-gray-800 shadow-sm">{typeLabel}</span>
        {isYouth && (
          <button
            onClick={toggleSave}
      ...
      <div className="flex flex-1 flex-col p-4">
        <div className="mb-2 flex items-center gap-2 text-xs text-gray-400">
          <span>{call.is_online ? t('common.online') : call.location ?? '—'}</span>
        ...
        <h3 className="line-clamp-2 font-semibold leading-snug group-hover:text-brand-600">{call.title}</h3>
        {call.subtitle && (
          <p className="mt-1 line-clamp-2 text-sm text-gray-500 dark:text-gray-400">{call.subtitle}</p>
        )}

        <div className="mt-auto flex items-center justify-between pt-4 text-xs">
          <span className="text-gray-400">
            {t('common.deadline')}: {formatDate(call.application_deadline, lang)}
          </span>
